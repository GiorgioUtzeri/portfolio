...
async function loadTranslations() {
  const fr = await fetch('src/fr.json').then(res => res.json());
  const en = await fetch('src/en.json').then(res => res.json());
  const it = await fetch('src/it.json').then(res => res.json());
  return { fr, en, it };
}
...
function changeLanguage(lang) {
  document.querySelectorAll('[data-translate]').forEach(el => {
    ...
    el.innerHTML = translations[lang][key];  // Utilisation de innerHTML
  });
}